...
public class SearchEtudaint extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        ArrayList<Etudiant> etudiants = Connect.toEtudiants(Connect.select(
                "SELECT E.nom, E.prenom, E.CNE, E.telephone, F.libelle as 'feliere', E.feliere as 'feliereId', F.departement as 'departementId', F.departement  FROM  etudiant E INNER JOIN feliere F on E.feliere = F.id WHERE nom like '%"
                        + query + "%' or prenom like '%" + query + "%' or cne like '%" + query + "%';"));
